...
from sklearn.metrics import precision_score, balanced_accuracy_score, recall_score, f1_score, confusion_matrix  # Métricas de evaluación
...
def guardar_model(path: str, estimator: GridSearchCV):
    ...
    Args:
        ...
        estimator (GridSearchCV): Modelo entrenado a guardar
    """
...
def main():
    ...
    test_df = cargar_data(os.path.join("files/input/", "test_data.csv.zip"))
    
    # Cargar los datos de entrenamiento desde el archivo comprimido
    # Estos datos se usarán para entrenar y optimizar el modelo
    train_df = cargar_data(os.path.join("files/input/", "train_data.csv.zip"))
    ...
    guardar_model(os.path.join("files/models/", "model.pkl.gz"), estimator)
    ...
    with open(os.path.join("files/output/", "metrics.json"), "w") as file:
